...
import { MarkAsUnsolvedRequest } from '@/gen/api';
import queryClient from '@/lib/react-query';
import { getErrorStatus } from '@/utils/error';

export const usePutPostAsUnsolved = () => {
  const toast = useToast();

  const mutation = useMutation({
    mutationFn: async (req: MarkAsUnsolvedRequest) => {
      const res = await postProblemApi.markAsUnsolved(req);
      return res.id;
    },
    onError: (error) => {
      console.error(error);
